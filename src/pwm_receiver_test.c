...
void init_TIMER1(){
	...
	TCCR1B |=  1<<CS11; //prescaler set to 1024
	TCNT1 = 0;//initialize counter
}

void init_PWM_interupt(){
	DDRC &= ~((1<<DDC0) | (1<<DDC1)| (1<<DDC2)| (1<<DDC3)); //enable pin PC1 & PC0 as input
	PORTC = (1<< PORTC0) | (1<< PORTC1); //enable pull-up resistor on pin PC1 & PC0
	PCICR |= 1<<PCIE1; // enable pin change interrupt 1
	PCMSK1 |= (1<<PCINT8) | (1<<PCINT9); //enable pin change interrupt on the corresponding I/O (PC0 & PC1)
