...
#include "transmitter.h"
	...
	void portInitTx(void)
	{
		DDRF &= ~_BV(4);					//nIRQ input to indicate previous transmission ended.
		DDRF |= _BV(5) | _BV(6) |  _BV(7);	//Put transmitter's SPI interface on port D.
	}
	...
	uint16_t writeCommandTx(uint16_t cmd)
	{
		//NOTE: The commands are 16 bit, SPI is 8 bit. Hence, need to do SPI manually.
		  ...
		  PORTF &= ~_BV(7); 		//Set SCLK low.
		  PORTF &= ~_BV(5);			//Set slave select low to select the RFM12B module.
		  for(i=0 ; i<16 ; i++)
		  {
			if(cmd & 0x8000)		//If the MSB of the command is high...
			{
				PORTF |= _BV(6);	//set MOSI high.
			}
			else					//Else...
			{
				PORTF &= ~_BV(6);	//set MOSI low.
			} 
	 

			PORTF |= _BV(7);		//Set SCLK high to clock the RFM12B module.
			temp <<= 1;				//Shift temp left by 1.

			if(PINE & _BV(6))		//If MISO is high...
			{
				temp |= 0x0001;		//set the LSB of temp to 1. Implied else means it is set to 0.
			}
			PORTF &= ~_BV(7); 		//Set SCLK low;
		  ...
		  PORTF |= _BV(5);			//Set slave select high to deselect the RFM12B module.
		  return(temp);				//Return the contents of the status register if it was read.
	...
	void sendByte(uint8_t byte)
	{
		while(PINF & _BV(4));		//Test value of pin F4 (nIRQ). Wait for previous transmission to end.
		writeCommandTx(0xB800 + byte);	//Write byte to the transmitter register for transmission.
	} 
#else
  #error "Don't know how to set up RF transmitter on the target device. Please update transmitter.c."
#endif